...
export default function BooksSection() {
  ...
  // Fetch the catalogue plus the signed-in student's role. The page sits
  // behind RequireAuth, so /auth/me/ is authoritative for is_librarian
  // (the stored session can be stale right after an admin grants the role).
  useEffect(() => {
    let cancelled = false;
    apiRequest<ApiBook[]>("/library/", { headers: authHeaders() })
      .then((data) => {
        if (!cancelled) setBooks(data.map(toBook));
      })
      .catch((err: unknown) => {
        if (!cancelled) {
          setLoadError(
    ...
    apiRequest<Student>("/auth/me/", { headers: authHeaders() })
      .then((me) => {
        if (!cancelled) setIsLibrarian(me.is_librarian === true);
      })
      .catch(() => {
        // The page is already gated; just default to non-librarian.
      });
  ...
  return (
    <section id="books" className="scroll-mt-20 flex flex-col gap-6">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {visibleBooks.map((book) => (
            <BookCard
              ...
              onDelete={isLibrarian && !deleting ? handleDelete : undefined}
            />
          ))}
        </div>
      ) : (
      ...
      {/* Librarian upload panel — non-librarians see the read-only note */}
      {isLibrarian ? (
        ...
        <div className="flex items-center gap-3 rounded-3xl glass-soft p-5 shadow-card sm:p-6">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-600">
              Only Librarians can add, upload and update books. Ask the library desk if something is missing.
            </p>
          </div>
        </div>
      )}
